...
<div>
  <div v-for="oneuser in alllist">
    <div v-for="(order,index) in oneuser.orderList">
      <div class="card" v-if="order.orderStatus==0?true:false">
        ...
        <div class="btn frigth" @click="sendOut(oneuser,order.orderId)">
          确认发货
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    sendOut(user,orderId) {
      ...
      axios.post('/users/sendout',params).then(response => {
        let res = response.data;
        console.log(res);
      });
      this.getallmessge();
